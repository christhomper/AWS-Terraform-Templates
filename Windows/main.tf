...
resource "aws_security_group" "allow_ssh_rdp_and_http" {
  name        = "allow_ssh_rdp_and_http"
  description = "Allow ssh rdp and http inbound traffic"
  vpc_id      = aws_vpc.CT_VPC.id
...
resource "aws_launch_configuration" "CT_Windows" {
  name                 = "CT_Windows"
  image_id             = "ami-03bd50811290e1bbd"
  instance_type        = "t2.micro"
  key_name             = "us-east-1"
  security_groups      = [aws_security_group.allow_ssh_rdp_and_http.id]
